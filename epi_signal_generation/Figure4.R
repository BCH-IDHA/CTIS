...
#TJV: read in the data matrices used for the heatmap

#df_block_1 <- readRDS("../datafolder/benchmark/20210506_df_block_1.rds")

#df_block_2 <- readRDS("../datafolder/benchmark/20210506_df_block_2.rds")

#df_block_3 <- readRDS("../datafolder/benchmark/20210506_df_block_3.rds")

#df_block_4 <- readRDS("../datafolder/benchmark/20210506_df_block_4.rds")

#df_block_5 <- readRDS("../datafolder/benchmark/20210506_df_block_5.rds")

df_raw <- readRDS("../datafolder/benchmark/20210506_df_raw.rds")

df_age <- readRDS("../datafolder/benchmark/20210506_df_age.rds")

df_sex <- readRDS("../datafolder/benchmark/20210506_df_sex.rds")

df_pred_wk_all <- readRDS("../datafolder/benchmark/pred_test_pos.rds")
...
df_sig_wk <- df_pred_wk_all %>%
  ...
  tq_mutate(
    ...
    col_rename = paste0("m7_", colnames(df_pred_wk_all)[2:5])) %>%
  
  # create percentages of weight an unweighted, daily and weekly
  mutate_at(vars('m7_test_pred_wt'), list(m7_test_pred_frac_wt=~./m7_weight_rw)) #%>%

df_raw_pred <- df_country %>%
  #left_join(df_data) %>%
  left_join(df_sig_wk) %>%
  left_join(df_bench) %>%
...
plot_corr_hist <- df_anno_corr_1_2 %>% 
  bind_rows(df_anno_corr_1_3, df_anno_corr_1_4, df_anno_corr_1_5) 

corr_hist <- ggplot(data=plot_corr_hist) +
  geom_histogram(aes(x=estimate, y=..count.., fill=mod_type), alpha=0.5, binwidth=0.1) +
  theme_minimal() +
  facet_grid(~mod_type)
corr_hist

df_anno_corr_all <- df_anno_corr_1_2 %>% 
  bind_rows(df_anno_corr_1_3, df_anno_corr_1_4, df_anno_corr_1_5) %>%
...
df_anno_corr_est <- df_anno_corr_1_2 %>% 
  bind_rows(df_anno_corr_1_3, df_anno_corr_1_4, df_anno_corr_1_5) %>%
...
df_anno_corr_pred <- df_raw_pred %>%
  dplyr::select(country_agg, iso_dt, m7_test_pred_frac_wt, bench_case) %>%
  dplyr::filter(!is.na(m7_test_pred_frac_wt) & !is.na(bench_case)) %>%
  nest(data = -country_agg) %>% 
  mutate(test = map(data,
                    ~ cor.test(.x$m7_test_pred_frac_wt, .x$bench_case, 
                               method="spearman", exact=FALSE)),
         tidied = map(test, tidy)) %>% 
  unnest(tidied) %>%
  mutate(mod_type = "sig1toPred")


df_anno_corr_cp <- df_anno_corr_1_2 %>%
  bind_rows(df_anno_corr_1_3, df_anno_corr_1_4, df_anno_corr_1_5) %>%
  dplyr::select(country_agg,mod_type,estimate) %>%
...
#Final Figure
par_row_split <- factor(df_anno_reg$region, levels=c("Europe","Americas","Africa","Asia and Oceania"))
# size of panels
par_width <- 3
par_height <- 40
...
anno_left = rowAnnotation(
  ...
  Pct = anno_barplot(df_anno_rep$pop_pct, gp = gpar(fill = 0)),
...
png("../Figure4_11_29_21_test.png", 
    width=40, height=45, units="cm", res=500)
ht_list =  h1 + h2 + h3 + h4 + h5 + hC 
draw(ht_list,
     
     # titles
     #column_title = "Global Symptom Survey vs Benchmark Signals",
     column_title_gp = gpar(fontsize = 16),
     
     # spacing
     ht_gap = unit(c(3,3,2,2,2), "mm"), 
     
     # annotation legend list
     annotation_legend_side = "right",
     annotation_legend_list = anno_lgd)
dev.off()
